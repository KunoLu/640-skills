...
This unreleased branch carries 14 bundled / 19 external Skills and on-demand SBTD setup. P1-04 provides explicit Codex project graph/MCP wiring; Graft installation remains separately confirmed. OMP wiring, complete root-installer forwarding, Windows-native proof, cleanup/recovery and full v2 release remain staged. Real-project deployment and legacy retirement require their independent authorization.

P1-01 supplies the internal argument and exchange-contract modules (`scripts/onboard_arguments.py`, `scripts/onboard_contracts.py`, `onboard-contracts.schema.json`). P1-12 registers `migration --phase plan|apply|verify`; P1-04 adds the Codex deployment producer through existing init entry points. Cleanup, recovery and OMP deployment remain separately staged. Do not substitute contract fixtures, valid hashes, or a copied directory for executed migration or recovery evidence.
...
- GitNexus CLI installation and its dedicated MCP menu are retired; user-owned installations/configs are not removed. Python Codex wiring is available as described below; OMP wiring and complete wrapper forwarding remain separate tasks.
