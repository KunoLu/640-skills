...
P1-01 supplies the internal argument and exchange-contract modules (`scripts/onboard_arguments.py`, `scripts/onboard_contracts.py`, `onboard-contracts.schema.json`). P1-12 now registers `migration --phase plan|apply|verify`; cleanup, recovery and migration deployment producers remain separate, unregistered work. Do not substitute contract fixtures, valid hashes, or a copied directory for executed migration or recovery evidence.
