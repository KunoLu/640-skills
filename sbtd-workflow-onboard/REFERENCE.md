...
- Gitignored paths under `.trellis` that sit outside the known layout are incidental. Plan does not require approval for them, but the directory snapshot still contains them, so later whole-tree cleanup deletes them. `tasks`, `spec` and `lessons` stay in the approval closure even when gitignore matches them. A path outside that layout that git does not ignore stops the plan and is listed for an explicit decision; plan does not delete it.
...
```text
python <installed-onboard.py> migration --phase plan --projects-root <comma-separated-absolute-roots> --backup-root <external-private-vault> --custodian <actual-label> [--publication-decisions <private-file>] [--routing-approvals <private-file>] [--routing-approval-key <private-key-outside-vault>] --json
