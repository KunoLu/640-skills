...
(function () {
  ...
  // the compiled impeccable engine, which serializes it into
  ...
  // one owns) comes from the canonical source, the compiled engine's chrome inventory,
  ...
  // Add a surface in the engine's surface inventory, not here.
  ...
  // the compiled engine's server and listed in its AGENT_PHASES, which the
  ...
  // Insert mode helpers (mirror the compiled engine's insert helpers)
  ...
  function handleServerLost() {
    ...
    if (state === 'GENERATING' || state === 'CYCLING' || state === 'SAVING') {
      showToast('Live server connection lost. Your session is saved; reopen this page or run impeccable live-poll to continue.', 6000);
    }
    hideBar();
    hideHighlight();
    hideShaderOverlay();
...
void main() {
  ...
  const AGENT_DISCONNECTED_TIP = 'Agent disconnected - run impeccable live-poll to connect';
  ...
  function steerTimeoutMessage() {
    ...
    if (!agentPollingConnected) {
      return head + 'No agent is polling right now. Run impeccable live-poll, then send it again.';
    }
