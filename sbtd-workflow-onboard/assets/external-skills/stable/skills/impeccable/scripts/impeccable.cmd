...
call :validate_download_base
if errorlevel 1 goto download_base_invalid
...
rem Per-process staging names: two first-run downloads for the same version
rem must never truncate, verify, move, or delete each other's files.
set "staging=%RANDOM%%RANDOM%"
set "part=%cached%.%staging%.part"
set "sidecar=%cached%.%staging%.sha256"
if exist "%part%\" goto cache_write_failed
(type nul >"%part%") 2>nul || goto cache_write_failed
set "asset=impeccable-windows-%arch%.exe"
set "url=%IMPECCABLE_DOWNLOAD_BASE%/engine-v%version%/%asset%"
curl.exe -fsSL -o "%part%" "%url%" >nul 2>nul
...
curl.exe -fsSL -o "%sidecar%" "%url%.sha256" >nul 2>nul
if errorlevel 1 goto verify_refuse
set "expected="
set /p expected=<"%sidecar%"
...
certutil -hashfile "%part%" SHA256 >"%sidecar%" 2>nul
if errorlevel 1 goto verify_refuse
call :check_download
if errorlevel 1 exit /b 127
for /f "usebackq skip=1 delims=" %%h in ("%sidecar%") do if not defined actual set "actual=%%h"
del "%sidecar%" >nul 2>nul
...
:check_download
set "download_file=%~1"
if not defined download_file set "download_file=%part%"
...
rem Validate IMPECCABLE_DOWNLOAD_BASE under delayed expansion: !VAR! expands
rem at execution time, so a crafted value stays data instead of breaking out
rem of this check or the later parse-time %VAR% expansions. Allowlist URL
rem characters only; anything else (quotes, &, |, <, >, ^, space, %) refuses.
setlocal EnableDelayedExpansion
if "!IMPECCABLE_DOWNLOAD_BASE!"=="" endlocal & exit /b 1
echo(!IMPECCABLE_DOWNLOAD_BASE!| findstr /r /c:"[^A-Za-z0-9:/._-]" >nul 2>nul
...
:download_failed
del "%part%" >nul 2>nul
goto download_base_invalid_done
:download_base_invalid
echo impeccable: IMPECCABLE_DOWNLOAD_BASE is empty or contains unsupported characters; set it to a plain https base URL 1>&2
exit /b 127
:download_base_invalid_done
