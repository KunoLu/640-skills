...
写入 lesson 前必须先确定 lessons 分隔名，并把内容写在该名字的标记块内：

- 分隔名的自动来源只有 `<repo-root>/.trellis/.developer` 的 `name=`；该文件缺失且当前是 linked worktree 时，读主 checkout 的 `.trellis/.developer`；都读不到就停下来问用户，不得猜测。
