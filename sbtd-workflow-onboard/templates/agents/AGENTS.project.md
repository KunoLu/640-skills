...
lesson ID 用 `LESSON-YYYYMMDD-<name>-<slug>`，`<name>` 取分隔名原样，不得转换：标记块只隔离写入，不隔离 ID 命名空间，两人同日同 slug 会在同一 topic 文件产生重复 heading 与歧义的 index `detail` 锚点。`<slug>` 可含 `-` 而 `<name>` 不可，ID 中名字与 slug 的边界因此唯一。既有 ID 不重命名。
