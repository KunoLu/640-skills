...
P1-18 在同一 `scripts/` 内增加确定性路由与受保护交接库（`sbtd_task_routing.py`、`sbtd_handoff.py`），不注册新全局 CLI、daemon、journal 或 `onboard.py` 子命令。`TaskRouter` 把 host 已明确的意图、mode、推荐回应与确认标记转成 `RouteDecision`，不分类自然语言；新任务先确定 ID，续作先确定 task，再决定 mode。接受推荐保留理由和风险标识，拒绝以结构化 `mode_note` 去重；普通自由文本 note 仍合法，相同已持久化选择不重复确认。保存失败保持会话选择并报告未持久化，不回退旧 mode。`TaskStore.current_binding()`／`recovery_candidates()`／`rebind(...)` 提供分支恢复；重绑定须携带预期分支、理由、证据并经确认，不 checkout、不 stash、不重置历史。`HandoffStore` 仅在 pause／context-switch／branch-switch／context-pressure／manual 续接时触发，计数或 checking 不触发；task/session 退出独立锁存，手动请求不清除退出；只读、保护、分支、确认和脱敏门仍有效。新文件名使用完整任务 ID 的定长 SHA-256 摘要，既有可逆 hex 名称继续可读但不自动改名；文件名与正文任务 ID/root 共同验证归属。仅与该任务最新快照比较是否相同，旧 A 不抑制 A→B→A 的新快照；跨日不单独触发。提醒仍只取 7 天内按任务去重、root/分支匹配且未完成的快照，旧快照可手动恢复但绝不改写 task。handoff 不是 mode/status 事实源。完整返回状态和调用契约见 [REFERENCE](sbtd-workflow-onboard/REFERENCE.md#routing-and-handoff-helpers-p1-18)。Agent 的完整方法执行仍须真实 host 证明，不因这些 helper 声称自动 SessionStart 恢复、真实迁移或完整发布已通过。

P1-19 在 `scripts/` 内增加按需 developer 身份库（`sbtd_identity.py`），同样不注册新全局 CLI、daemon、initializer 或身份数据库。`DeveloperStore(root, read_only=False)` 提供只读 `resolve()`／`plan(name)` 与确认门控的 `ensure(name, confirmed=False, protect=False)`，统一返回冻结 `IdentityResult`（status／name／source／path／first_write_eligible／topology／reason／needs_protection／completed_steps，可 `dataclasses.asdict` 导出）。本地 `.sbtd/developer` 合法值（UTF-8、单条 `name=`、`^[a-z0-9]+$` 原样匹配单一 `validate_developer_name`）直接胜出且不再查 Git；仅本地确实缺失后才核验真实 Git 根、git-dir/common-dir 与 NUL 分隔 worktree registry，已验证 linked worktree 只读同仓主 checkout 当前身份、不复制回本地；现存异常（重复声明、非法值、错误类型、symlink、不可读或父路径不安全）是 conflict 而非缺失，Git 未知或不可用是 blocked 而非非 Git 证明；不从 Git／OS／环境／历史目录猜作者，普通 resolve/plan 不读旧 `.trellis/.developer`。首次建立只允许正常缺失目标：窄 `/.sbtd/` ignore 保护与名字授权分开确认，仅写 `name=<name>\n` 并回读验证，同名幂等、异名 conflict、并发胜者不覆盖、失败如实返回 completed_steps。`onboard.py` 仅经显式 `--developer <name>` 消费：`check`／`plan` 只读并在单 JSON 中展示逐项目 `developerPlan`（请求名、来源、目标、状态、needsProtection 等），conflict／blocked／needs-* 或无项目范围时 exit 2、绝不默认 cwd 或 HOME；`init`／`reset`／`init-projects` 要求 `--developer` 与 `--yes` 同现，在任何全局或项目写入之前完成全批次身份 preflight，冲突先于副作用拒绝，写后身份结果按状态返回：failed 为 exit 5，conflict／blocked／needs-* 为 exit 2，两类均保留部分结果；无 `--developer` 行为完全不变，reset 不覆盖既有身份。根安装器不新增 developer 专属 flag，已实现公开参数转发；旧身份真实迁移仍是独立 P1-12 门，不因本 helper 宣称迁移完成。这不代表 Windows、真实 host、全量验证或发布已通过。
