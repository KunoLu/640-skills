...
13. 运行验证：
  ...
  - 迁移交接复用HandoffStore格式和publication批准，不自动摘要或选择active任务；核对固定旧session JSON、完整journal来源绑定、未完成任务覆盖、ignore保护与恢复原件。非API辅助报告的类型化内容／来源验证不能替代真实API smoke；未知格式必须明确拒绝，不把sha匹配等同于内容通过。
