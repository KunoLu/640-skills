...
- 本文件是 Orca 自动化 `SBTD Workflow Tools Version Check` 的版本化 prompt 源；修改后必须同步更新同名 live automation，并验证两者内容一致。
...
12. 如果模板内容有更新，必须评估 `README.md` 和 `README.html` 是否需要同步；只有影响 `AGENTS.md` 规定的 README 同步范围时才同步更新，否则在最终输出说明跳过原因。
13. 运行验证：
   - `git status --short`
...
14. 最终输出必须说明：发现的版本区间、修改的文件、验证命令和结果、跳过项及原因、剩余风险、`rtk` 使用状态。再次强调：不要 commit，不要 push，不要把最新版本写回 `ENTRYPOINT.md`。
