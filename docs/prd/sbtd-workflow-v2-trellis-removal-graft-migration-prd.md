...
| P1-06 | P1 | 多项目／worktree 隔离；禁止父目录 Graft 入口 | P1-02、P1-04、P1-05 | AC-09/13；[实施设计](sbtd-workflow-v2-worktree-graft-isolation.md)。只对显式仓根调用；未选 sibling 零写入；多仓理解=逐仓调用后只读汇总；证明未对父目录 init/build/MCP。Legacy/Refactoring/DDIA门已确认，进入实现 | AFK | in-progress | — |
