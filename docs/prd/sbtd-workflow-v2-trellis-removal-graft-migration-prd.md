...
|---|---|---|---|---|---|---|---|
| P2-01 | P2 | 冻结迁移范围、备份责任及保留安排 | P1-16 | AC-37；[冻结协议](sbtd-workflow-v2-migration-scope-freeze.md)。已接受逻辑名 `demo`/`main`、两host、custodian `kuno`、源仓不纳入。HOME政策：demo 正式切换到新建的独立 Codex／OMP HOME 与独立 Skill 根；当前共享 live HOME／Skill 根保持不动，且不作为 demo 切换目标。P2-02 在隔离副本＋用户确认后的隔离根演练、不改 live。闭包仅 demo；第 6 节第 3 项仍未满足：隔离政策已接受，精确隔离根路径仍为 agent-proposed / pending user confirmation（TEMP 下候选目录为 Agent 自行选择，用户未确认），不是整批纳入，也不是已创建目录。用户先前「HOME 只服务 demo」不得写成盘点结论。未选平台只盘点。backup_root：TEMP 父目录下专用子目录，只留备份／私有记录，不得与受管目标互相包含；精确路径只进私有层并已回读；父目录本身不是 backup_root；通用下载目录仍拒绝。正式隔离根候选路径为 agent-proposed / pending user confirmation（Agent 自行选择 TEMP 下目录，用户未确认），在 backup_root 之外；目录尚未创建；先前误建的空占位已删除。已接受默认保留；拟清理本批只盘点、全局卸载 not-allowed。精确项目根已写入私有层并回读（路径不进本文件；用户确认原文「项目根候选可用，仍是 main」，当时非 git 实测）。live HOME／Skill 根（只读盘点对象）已写入私有层并回读；隔离根候选路径为 agent-proposed / pending user confirmation（尚未创建，用户未确认具体 TEMP 目录；路径不进本文件；「HOME 候选都可用」是当时 live 候选回读，不是正式切换目标）。旧 Trellis：用户原文存在，协议层类别为仓库内 `.trellis` 目录；版本未说明、不猜；细节只进私有层。第4节已开始并完成本轮只读记录（私有层；路径/HEAD/工作区/目录结构/指纹不进本文件）。盘点确认旧 Trellis 存在，协议层类别仍为仓库内 `.trellis` 目录。项目内有受管 `.codex`/`.omp` Trellis 接线；身份文件 `.trellis/.developer` 存在、`.sbtd/developer` 不存在，未改写。HOME 只读已确认根：Skill 根可见 trellis/gitnexus/onboard，未见 graft 名；Codex HOME hooks 状态有本批以外项目痕迹，不扫描不纳入。用户原文「仍只迁 demo，那些痕迹忽略」（2026-09-22T17:48:03+08:00）：不扫描、不纳入、不扩大本批。用户原文「按你推荐和建议的执行」（用户事件未定时；不得用 11:01:55 充当用户事件时间）：隔离政策为明确隔离根，不把当前共享 live HOME 改成隔离根；live HOME 保持不动且不作为 demo 切换目标；不把 live HOME 改写成独用；全局卸载仍 not-allowed。review 发现／Agent 校正（不是 HITL、不是用户授权证据；记录时刻 11:06:58）：正式隔离根不得放在 backup_root 内（协议已要求不得与受管目标互相包含）。用户从未说过「立即回滚正式隔离根建在 backup_root 下」，也未在 11:06:58 提供该裁决。§6.3 仍未满足：精确隔离根路径为 agent-proposed / pending user confirmation。P2-01 不创建正式 HOME；P2-02 获单独授权后再建目录和填充。第 6 节第 5 项独立 review 为 blocked/unavailable（同一实现会话只读复审不算；subagent 不可用，须不同 reviewer／会话审过当前快照后才能关闭）。用户原文「需要触碰这个任务」及预期迁移行为（用户事件未定时；不得用 18:26:22 充当用户事件时间）：发现 `00-bootstrap-guidelines` 则跳过该任务内容，不迁成新任务、不要求其 handoff；理解其行为是读仓库代码后落地规范，并在后续已授权窗口模拟写入 `docs/spec/`。本批不以空模板直拷冒充已完成规范；原 spec 仍私有保全。不在 P2-01 写项目。此条只解除该起步任务因缺 handoff 造成的相关迁移 blocked。当前 v2 新项目 init 不会自动做同类 spec 落地（仅用户明确要求规范初始化时才有条件任务）。未复制原件。不批准apply/deploy/cleanup/sync/tag/备份销毁 | HITL | in-progress | — |
...
2. P2-01 仍 in-progress。已接受逻辑名 `demo`（声明 `main`）、Codex+OMP、custodian `kuno`、源仓不纳入；HOME 政策为 demo 正式切换到新建的独立 Codex／OMP HOME 与独立 Skill 根；当前共享 live HOME／Skill 根保持不动，且不作为 demo 切换目标。P2-02 隔离演练不改 live。闭包仅 demo；第 6 节第 3 项仍未满足：隔离政策已接受，精确隔离根路径仍为 agent-proposed / pending user confirmation（TEMP 下候选目录为 Agent 自行选择，用户未确认），不是整批纳入，也不是已创建目录。用户先前「HOME 只服务 demo」不得写成盘点结论。未选平台只盘点；默认保留；拟清理本批只盘点且全局卸载 not-allowed。backup_root 为用户点名 TEMP 父目录下专用子目录（父目录本身不是 backup_root；通用下载目录仍拒绝），只留备份／私有记录，不得与受管目标互相包含，精确路径只进私有层并已回读。正式隔离根候选路径为 agent-proposed / pending user confirmation（Agent 自行选择 TEMP 下目录，用户未确认），在 backup_root 之外；目录尚未创建；先前误建的空占位已删除。精确项目根与各 host HOME／Skill 根已写入私有层并回读（路径不进本文件）。旧 Trellis：用户原文存在，协议层类别为仓库内 `.trellis` 目录，版本未说明、不猜。第 3 节政策字段均有用户原文；有效 HOME 的精确隔离根路径仍为 agent-proposed / pending user confirmation，故该字段未齐。第 4 节已开始并完成本轮只读记录（私有层）。盘点确认旧 Trellis 存在（仓库内 `.trellis`）。Codex HOME hooks 状态有本批以外项目痕迹。用户原文「仍只迁 demo，那些痕迹忽略」：不扫描、不纳入、不扩大本批。用户原文「按你推荐和建议的执行」（用户事件未定时；不得用 11:01:55 充当用户事件时间）：隔离政策为明确隔离根，不把当前共享 live HOME 改成隔离根；live HOME 保持不动且不作为 demo 切换目标；全局卸载仍 not-allowed。review 发现／Agent 校正（不是 HITL、不是用户授权证据；记录时刻 11:06:58）：正式隔离根不得放在 backup_root 内（协议已要求不得与受管目标互相包含）。用户从未说过「立即回滚正式隔离根建在 backup_root 下」，也未在 11:06:58 提供该裁决。§6.3 仍未满足：精确隔离根路径为 agent-proposed / pending user confirmation。P2-01 不创建正式 HOME；P2-02 获单独授权后再建目录和填充。第 6 节第 5 项独立 review 为 blocked/unavailable（同一实现会话只读复审不算；subagent 不可用，须不同 reviewer／会话审过当前快照后才能关闭）。用户原文「需要触碰这个任务」及预期迁移行为（用户事件未定时；不得用 18:26:22 充当用户事件时间）：发现 `00-bootstrap-guidelines` 则跳过该任务内容，不迁成新任务、不要求其 handoff；理解其行为是读仓库代码后落地规范，并在后续已授权窗口模拟写入 `docs/spec/`。本批不以空模板直拷冒充已完成规范；原 spec 仍私有保全。不在 P2-01 写项目。此条只解除该起步任务因缺 handoff 造成的相关迁移 blocked。当前 v2 新项目 init 不会自动做同类 spec 落地（仅用户明确要求规范初始化时才有条件任务）。点名项目根的提前只读属门禁外观察，不得抄入充数。不得扫描未点名 HOME。尚未批准 apply／部署／cleanup／sync／tag 或备份销毁。
...
| 2026-09-23T11:01:55+08:00 | P2-01 隔离政策写入记录（非用户事件） | 记录/写入时刻 11:01:55，不是用户消息发生时钟。用户原文「按你推荐和建议的执行」保持未定时。裁决内容不变：本批仍仅逻辑名 demo；盘点他项 traces 不扫描、不纳入、不扩大。demo 正式切换到新建的独立 Codex HOME、独立 OMP HOME、独立 Skill 根；当前共享 live HOME／Skill 根保持不动，且不作为 demo 的切换目标。不把当前共享 live HOME 改成隔离根，也不把 live HOME 改写成独用。本条只冻结隔离政策。当时空占位误放在 backup_root 内，位置由后续事件校正；当时不得声称 §6.3 已满足。P2-02 才在隔离副本＋隔离根演练、不改 live。P2-03 不停用／不卸载共享 live HOME 上的旧工具。全局卸载仍 not-allowed。独立 review 未做。未 apply。P2-01 仍 in-progress。 |
| 2026-09-23T11:06:58+08:00 | P2-01 隔离根位置校正记录（review 发现／Agent 校正） | 不是 HITL、不是用户授权证据。记录/写入时刻 11:06:58，不是用户消息发生时钟。用户从未说过「立即回滚正式隔离根建在 backup_root 下」，也未在 11:06:58 提供该裁决。先前误把 watchdog 建议写成用户原文，该记法作废。Agent 依据协议已接受的「backup_root 不得与受管目标互相包含」删除 backup_root 内空占位，并把 planned 路径改到同一 TEMP 父目录下、backup_root 之外。当时又在该处创建空目录，超出 P2-01 只读冻结范围，由后续事件删除。backup_root 只留备份／私有记录。不得把本条当成用户授权。未写 live HOME。独立 review 未做。未 apply。P2-01 仍 in-progress。 |
| 2026-09-23T11:12:13+08:00 | P2-01 隔离根改为 planned／尚未创建（review 发现／Agent 校正） | 不是 HITL、不是用户授权证据。记录时刻 2026-09-23T11:12:13+08:00；删除空目录时刻 2026-09-23T11:10:38+08:00。P2-01 不创建正式 HOME；11:01:55 不是用户消息发生时钟。已删除 backup_root 外误建的空目录。当时把三条路径记为 planned / not yet created，并误称 §6.3 已满足；后续改为 agent-proposed / pending user confirmation。用户原文「按你推荐和建议的执行」保持未定时。不得把本条当成用户授权。未写 live HOME。独立 review 未做。未 apply。P2-01 仍 in-progress。 |
| 2026-09-23T11:16:19+08:00 | P2-01 记法校正（review 发现／Agent 校正：此前归因错误） | 不是 HITL、不是用户授权证据。先前误把 watchdog 建议写成用户原文，该记法作废。冻结协议 §3.1.17、表格共享闭包栏、本表 11:06:58 事件及私有记录相应条改为 review 发现／Agent 校正，不能作为 HITL 或用户授权证据。用户从未说过「立即回滚正式隔离根建在 backup_root 下」，也未在 11:06:58 提供该裁决。用户原文「按你推荐和建议的执行」保持未定时；11:01:55 只是事后写入时钟。未 apply。当时写「不进入 commit／review」被后续 advisory 覆盖。P2-01 仍 in-progress。 |
| 2026-09-23T11:20:09+08:00 | P2-01 隔离根改标 agent-proposed／待确认（review 发现／Agent 校正（非 HITL、非用户授权证据）） | 不是 HITL、不是用户授权证据。先前误把 watchdog 建议写成用户消息，该记法作废。Agent 校正：提前建的空正式根须删除；路径标为 agent-proposed / pending user confirmation（TEMP 下「持久目录」是 Agent 自行选择）；待 P2-02 单独授权后再创建。用户从未说过这些具体操作句。磁盘回读候选路径不存在。隔离政策仍以用户原文「按你推荐和建议的执行」为准（未定时）。§6.3 因精确路径未经用户确认而仍未满足。P2-01 不创建正式 HOME。未写 live HOME。未 apply。当时误把同一会话只读复审当成独立 review 入口，已由后续事件更正为 blocked/unavailable。P2-01 仍 in-progress。 |
| 2026-09-23T11:21:49+08:00 | P2-01 同一会话只读复审记录（independent review: blocked/unavailable） | 本会话无独立 subagent。同一实现会话内的只读复审不是独立 review，不得宣称独立 review 已做，不得写「无剩余文档 P0／P1」，也不能关闭第 6 节第 5 项。该复审曾被误称为独立 review，该记法作废。第 6 节第 5 项独立 review 为 blocked/unavailable。未 apply。P2-01 仍 in-progress。 |
| 2026-09-23T11:26:51+08:00 | P2-01 独立 review 门标 blocked/unavailable | review 发现／Agent 校正，不是 HITL、不是用户授权证据。subagent 不可用。没有不同 reviewer／不同会话实际审过当前快照。第 6 节第 5 项独立 review 为 blocked/unavailable。不得把 11:21:49 只读复审当成独立 review，也不得据此写「无剩余文档 P0／P1」。只有不同 reviewer／会话实际审过该快照后才能关闭此门。未 apply。P2-01 仍 in-progress。 |
