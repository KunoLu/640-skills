...
| P2-02 | P2 | 隔离副本迁移及可调用恢复演练 | P2-01 | AC-17/18/35/36；[演练协议](sbtd-workflow-v2-isolated-recovery-drill.md)。隔离 apply 已完成并曾恢复后按带 deployment 的新 plan 再次 apply。用户要求保留现场。官方 verify 返回过 `verified`，但不是事前授权，不能据此 cleanup。旧工具只读探测可调用，写操作仍缺证据。未 cleanup，未启动 host。任务 PR #73 已合并，merge `2e22adc1a2644c99cbbe830c13ffe99881b1afaa`。演练协议 §5.3 的 cleanup 授权仍未满足，§5.5 要求状态 PR 闭环后才标 done，完成时间不预填 | HITL | checking | — |
...
5. P2-02 为 checking。任务 PR #73 已于 GitHub `2026-09-24T14:27:12Z` 合并，merge `2e22adc1a2644c99cbbe830c13ffe99881b1afaa`。完成时间不预填。用户接受凭据风险，不证明令牌已撤销。隔离副本已 apply，现场按 `2026-09-24T17:59:27+0800` 用户要求保留。官方 verify 不是事前授权，不能据此 cleanup。演练协议 §5.3 仍未满足。旧工具写操作仍按缺证据作安全拒绝。未写 live。禁止启动 host。P2-03 仍 planned。不标 done。
