...
6. Then assess clarification. Read facts the project can answer. If requirements are clear, explain why full `grill-with-docs` was not needed and proceed. For consequential domain ambiguity, explain the unresolved point and ask about grill. Declining a method does not authorize guessing required answers. Every completed full grill gets an independent DDD boundary review before confirmation/design; interview-time modeling is not that review. Report missing Skills honestly; unresolved substantive ambiguity still blocks dependent decisions.
