...
- 新边界：允许在本配置源仓库实施计划任务及隔离测试；每项从最新 main 建任务分支。按用户最新授权，当前及后续任务循环独立 review 至不再发现 P0／P1 问题即可推进；P2 及更低级别记入根目录 `findings.log`，标明发现任务，留待用户评估，不阻塞本轮推进。已收到 advisor 同样按有效性与严重级别裁决；详见 D-IMP-13。用户已授权通过 PR 使用 `--admin` 合并。
...
## D-IMP-13：审查门槛与低优先级发现台账

- 日期：2026-09-18；适用范围：当前 P1-01 及本计划所有后续任务，包括任务收尾的状态文档 PR。
- 依据：用户明确将“review 不得有任何 findings”改为“不再发现 P0 和 P1 即可继续推进”，并要求 P2 及更低级别统一记录在根目录 `findings.log`，注明发现任务。
- 决定：有效 P0／P1 必须修复并在新证据基础上独立复审；当前审查不再发现 P0／P1、其他必需验收通过后即可进入 PR／合并流程。不得把已知高优先级问题随意降级以绕过门槛。
- 低优先级：P2／P3 等非阻塞问题逐条记录发现任务 ID、原审查级别、来源／轮次、发现时 revision 与位置、问题／影响、建议和当前状态；默认 `deferred`，后续由用户评估是否处理。已经修复的历史低优先级问题可回填为 `fixed`，不回滚其修复；撤回或无效建议标明裁决，不伪装成已修复。
- 边界：该调整只改变独立 review 的推进条件，不豁免任务验收、必要测试、数据安全／授权、适用 Book gate、真实合并证据或 D-IMP-02 的两层 PR 收尾顺序。这里的 P0／P1／P2 是 finding 严重级别，不改变主 PRD 的 P0／P1／P2／P3 任务阶段。
- 文档维护：主 PRD §14.1 与本记录同步采用新门槛；历史“零新发现”仅保留为当时事实，不作为今后的执行要求。不修改全局 AGENTS、Onboard 模板或 live automation 来推广这项计划内授权。

## D-IMP-14：P1 累计十项后的 findings 评估确认门

- 日期：2026-09-18；依据：用户在P1-17实施期间明确要求P1做完第10个任务后暂停后续推进，评估全部findings并先列出优先修复建议，等待确认。
- 决定：按P1累计闭环的任务数量计数；第10项的实现/状态PR及main/分支清理完成后触发。当前顺序对应P1-06，不等于编号P1-10；当前已有P1-01/02/03三项闭环。
