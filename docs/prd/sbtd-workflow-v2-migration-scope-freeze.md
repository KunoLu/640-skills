...
本仓库仍不创建 `.trellis/`、`ai/tasks/` 或 `.feature`。真实目标项目的新目录不自动变成本仓库目录。已记录用户政策原文（见第 3 节）。精确项目根与各 host 有效 HOME／Skill 根路径仍待私有层回读。当前会话环境变量不得自动当成真实 HOME。未点名 HOME 不扫描。
...
| 字段 | 必须由用户提供 | 当前值 |
|---|---|---|
| 至少一个实际项目根 | 用户提供绝对路径与当前分支或 `detached:<full-sha>` 或明确非 Git；**精确路径只进私有层**，本表只记逻辑名与声明分支 | 已接受逻辑名 `demo`，声明分支 `main`。精确路径、HEAD、工作区与远程跟踪不进本文件，**尚未私有回读**。**640-skills 不纳入。** |
| 每个项目的旧 Trellis 结构 | 用户说明或第 4 节授权盘点后记录；不猜版本。协议层只记类别／是否存在，细节进私有层 | pending。用户未说明版本或结构。字段未齐前不得盘点填此栏。 |
| 验证 host | Codex、OMP 或两者 | 已接受：两者 |
| 有效 HOME / Skill 根 | 每个 host 的实际配置根；共用者须整批纳入或明确隔离。精确路径只进私有层 | **政策已接受：** 冻结 `demo` 当前实际使用的 Codex、OMP 配置根，作为本批正式切换目标。P2-02 才在隔离副本＋隔离 HOME 演练、不改 live；P2-03 才迁移已冻结批次并停用旧路由。正式切换不降格为隔离演练。**精确路径尚未私有回读。** 未扫描未点名 HOME。当前会话环境变量不得自动冻结。 |
| 共享批次闭包 | 同 HOME／Skill／全局入口的依赖项目名单，或隔离方案 | **已接受：** 本批授权项目仅逻辑名 `demo`。用户确认该实际 HOME 目前只服务 `demo`（试点独用）。未点名项目不扫描、不纳入。本批全局卸载仍 not-allowed。 |
...
| 私有 `backup_root` | 仓库外专用目录；不得与受管目标互相包含。精确路径只进私有层 | **已接受：** 在用户点名的 TEMP 父目录下新建专用子目录作为 `backup_root`；父目录本身不是 `backup_root`。通用下载目录仍拒绝。精确路径只进私有层，已回读。尚未复制原件。 |
...
1. 项目：逻辑名 `demo`，声明分支 `main`。未写该项目。精确路径不进本文件，**尚未写入私有层**。
2. host：Codex 与 OMP。
3. HOME／Skill 根政策：冻结 `demo` 当前实际 Codex、OMP 配置根为本批正式目标；P2-02 隔离演练不改 live。精确路径不进本文件，**尚未写入私有层**。未扫描未点名 HOME。
4. custodian：`kuno`。
5. `backup_root`：通用下载目录已拒绝。已接受在用户点名的 TEMP 父目录下新建专用子目录；父目录本身不是 `backup_root`。精确路径只进私有层，已回读。尚未复制原件。
6. 保留政策：已接受协议默认（P3 两周观察 ∪ 正式发布后 14 天，复核 ≤30 天）。
7. 拟清理：已接受本批只盘点；项目内受管旧条目待授权盘点后才可列为 P2-05 候选；全局卸载 not-allowed。
8. 640-skills：不纳入。
9. 可选私有冻结文件：不需要。
10. 共享闭包：本批仅 `demo`；用户确认该实际 HOME 目前只服务 `demo`。未点名项目不扫描。
11. 未选平台：只盘点、不修改；依赖未解前禁止全局卸载。
12. 私有准备记录：已建在专用 `backup_root` 内并回读；精确路径不进本文件。

政策已收口，但精确项目根与各 host HOME／Skill 根仍未私有回读，第 3 节未齐，P2-01 保持 in-progress。未做授权盘点、未 apply、未 sync、未清理。曾有门禁外项目根读取（§3.2），不是盘点。
