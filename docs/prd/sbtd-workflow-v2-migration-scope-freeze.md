...
| 有效 HOME / Skill 根 | 每个 host 的实际配置根；共用者须整批纳入或明确隔离。精确路径只进私有层 | **政策已接受（用户原文「按你推荐和建议的执行」，用户事件未定时；11:01:55 仅为当时写入/记录时刻，不是用户消息发生时钟）：** `demo` 正式切换到新建的独立 Codex HOME、独立 OMP HOME、独立 Skill 根；当前共享 live HOME／Skill 根保持不动，且不作为 `demo` 的切换目标。P2-02 才在隔离副本＋用户确认后的隔离根演练、不改 live；P2-02 获单独授权后再建目录和填充。P2-03 只对已冻结 `demo` 批次及隔离根迁移／停旧路由；共享 live HOME 上的旧工具不停用、不卸载。正式切换不降格为隔离演练。先前「冻结当前实际 Codex／OMP 根为正式目标」已作废。用户确认原文「HOME 候选都可用」（2026-09-22T17:06:54+08:00）仍是当时 live 候选回读，不是正式切换目标。**隔离根精确路径仍为 agent-proposed / pending user confirmation**（Agent 自行选择 TEMP 下候选目录，用户未确认该具体路径；状态尚未创建；不得写入 `backup_root`）。先前误建的空占位均已删除。P2-01 不创建正式 HOME。第 6 节第 3 项因此仍未满足。live 根精确路径仍在私有层，仅作第 4 节只读盘点对象。未扫描未点名 HOME。当前会话环境变量不得自动冻结。 |
| 共享批次闭包 | 同 HOME／Skill／全局入口的依赖项目名单，或隔离方案 | **已接受，并经后续隔离 HITL 裁决（用户事件未定时；11:01:55 仅为当时写入/记录时刻）：** 本批授权项目仅逻辑名 `demo`。用户先前原文「该实际 HOME 目前只服务 demo」不得写成盘点结论。第 4 节盘点见 Codex HOME hooks 状态有本批以外项目痕迹。用户原文「仍只迁 demo，那些痕迹忽略」（2026-09-22T17:48:03+08:00）：不扫描、不纳入、不扩大本批。用户原文「按你推荐和建议的执行」（用户事件未定时）：隔离政策为明确隔离 HOME／Skill 根，**不把当前共享 live HOME 改成隔离根**，live HOME 保持不动且不作为 `demo` 切换目标。不把 live HOME 改写成独用。review 发现／Agent 校正（不是 HITL、不是用户授权证据；记录时刻 11:06:58）：正式隔离根不得放在 `backup_root` 内（协议已要求 `backup_root` 不得与受管目标互相包含）。用户从未说过「立即回滚正式隔离根建在 backup_root 下」，也未在 11:06:58 提供该裁决。第 6 节第 3 项仍未满足：隔离政策已接受，但精确隔离根路径仍为 agent-proposed / pending user confirmation（TEMP 下候选目录为 Agent 自行选择，用户未确认）。不是整批纳入，也不是已创建目录。未解共享痕迹 → 全局卸载仍 not-allowed。 |
...
| 私有 `backup_root` | 仓库外专用目录；不得与受管目标互相包含。精确路径只进私有层 | **已接受：** 在用户点名的 TEMP 父目录下新建专用子目录作为 `backup_root`；父目录本身不是 `backup_root`。通用下载目录仍拒绝。精确路径只进私有层，已回读。尚未复制原件。`backup_root` 只留备份／私有记录，**不得与受管目标互相包含**。正式隔离 HOME／Skill 根不得放进 `backup_root`。Agent 曾提议同一 TEMP 父目录下另一候选目录，用户未确认该具体路径；P2-01 不创建这些目录。先前误建的空占位已删除。候选路径只进私有层，标为 agent-proposed / pending user confirmation。 |
...
8. 640-skills：不纳入。
9. 可选私有冻结文件：不需要。
10. 共享闭包：本批仅 `demo`。用户先前「HOME 目前只服务 demo」不得写成盘点结论。未点名项目不扫描。第 6 节第 3 项仍未满足：精确隔离根路径仍为 agent-proposed / pending user confirmation。不是整批纳入，也不是已创建目录。
...
20. 隔离根改标 agent-proposed／待确认（记录时刻 2026-09-23T11:20:09+08:00；review 发现／Agent 校正：此前归因错误；不是 HITL、不是用户授权证据）：先前误把 watchdog 建议写成用户消息，该记法作废。Agent 校正：删掉提前建的空正式根；路径标为 agent-proposed / pending user confirmation（尤其 TEMP 下「持久目录」是 Agent 自行选择）；待 P2-02 单独授权后再创建。用户从未说过这些具体操作句。磁盘回读：三条候选路径及其父目录均不存在；backup_root 内旧占位也不存在。隔离政策（新建独立根、live HOME 不动）仍以用户原文「按你推荐和建议的执行」为准，保持未定时。第 6 节第 3 项因精确路径未经用户确认而仍未满足。P2-01 不创建正式 HOME。未写 live HOME。未 apply。
21. 同一会话只读复审记录（记录时刻 2026-09-23T11:21:49+08:00；**independent review: blocked/unavailable**）：本会话无独立 subagent。同一实现会话内的只读复审不是独立 review，不得宣称独立 review 已做，不得写「无剩余文档 P0／P1」，也不能关闭第 6 节第 5 项。该复审曾被误称为独立 review，该记法作废。第 6 节第 5 项独立 review 为 **blocked/unavailable**。未 apply。P2-01 仍 in-progress。
...
- 已确认 HOME／Skill 根：只读，未点名根不读。全局 Skill 根可见 `trellis-workflow`／`trellis-channel`／`gitnexus-*`／`sbtd-workflow-onboard`；未见名称含 graft 的 Skill。Codex HOME 配置已核实含 `mcp_servers.gitnexus`（凭据不进本文件）。OMP HOME **根目录没有** `mcp.json`；本轮 OMP HOME 一层证据是插件锁 `@kunolu/omp-sbtd`，**不得**据此把 GitNexus MCP 写成两套 HOME 根都有。OMP 更深 MCP 文件若存在，只进私有层，且不得与 Codex HOME 根配置并列。Codex HOME 的 hooks 状态里还有本批以外项目痕迹。用户原文「仍只迁 demo，那些痕迹忽略」（2026-09-22T17:48:03+08:00）：**不扫描、不纳入、不扩大本批**；当时不是隔离 live HOME，也不是把 HOME 改写成独用。全局卸载仍 not-allowed。后续隔离 HITL 见第 3.1 条第 16 点；本条只记录当时 live HOME 观察。
...
- 用户原文「按你推荐和建议的执行」（用户事件未定时；不得用 11:01:55 充当用户事件时间）：隔离政策为 `demo` 正式切换到新建独立 HOME／Skill 根；当前共享 live HOME／Skill 根保持不动、不是切换目标。当时空占位误放在 `backup_root` 内，已由后续校正删除。
- 隔离根位置校正记录（review 发现／Agent 校正；不是 HITL、不是用户授权证据；11:06:58 仅为当时写入/记录时刻）：planned 路径改到同一 TEMP 父目录下、`backup_root` 之外。用户从未说过「立即回滚正式隔离根建在 backup_root 下」。当时误建空目录，超出 P2-01 只读范围。
- 隔离根改为 planned／尚未创建（review 发现／Agent 校正；不是 HITL、不是用户授权证据；记录时刻 2026-09-23T11:12:13+08:00；删除空目录 2026-09-23T11:10:38+08:00）：已删除 `backup_root` 外误建空目录。当时仍把 Agent 自选路径写成已冻结 planned 根。
- 隔离根改标 agent-proposed／待确认（记录时刻 2026-09-23T11:20:09+08:00；review 发现／Agent 校正，不是 HITL、不是用户授权证据）：精确路径标为 agent-proposed / pending user confirmation，不满足第 6 节第 3 项。磁盘回读候选路径不存在。未写 live HOME。未 apply。
