...
If the user skips project-level `AGENTS.md`, provide the bundled template path `templates/agents/AGENTS.project.md` and ask them to confirm that this is the file they will use for manual setup.
...
Summarize the runtime, CLI tool, bundled skill, referenced skill, and manual MCP / conditional project checks. If npm is missing, ask for confirmation and run `python scripts/onboard.py ensure-npm --yes` before any CLI tool check or install. Do not proceed to `init` or `reset` until the user confirms install or explicitly chooses to skip missing optional items.
...
6. If project-level `AGENTS.md` is skipped:
