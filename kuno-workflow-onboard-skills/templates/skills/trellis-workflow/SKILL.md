...
- API、Web E2E、Mobile E2E 或 Hybrid E2E 调试轮次不要沉淀多份正式报告；一旦 Playwright 或 Maestro 运行产生 runner 原生报告，最后一次相关运行必须生成命名报告和同目录同 stem 的中文 Markdown 汇总。Playwright 的 Markdown 汇总必须跟随 `playwright-report-*.html` 的 stem，而不是 `results.json`、`junit.xml` 或默认 `index.html`。Markdown 汇总记录运行 case / spec / flow 列表、关联 BDD `.feature` 路径和场景名、总轮次、每轮失败 case / spec / flow、失败原因、修复动作、定点重跑、影响范围重跑和最终全量重跑结果；状态枚举值、命令、文件路径、case / spec / flow 名称和错误原文可以保留英文。
